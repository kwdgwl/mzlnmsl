...
def login(account, password, model):
    ...
    ses_data = json.dumps({"password": password, "info": headers['uuid'], "mobile": account, "type": model}, separators=(',',':'))
    ...
    if isiOS:
        if 'utoken' in headers:
            headers.pop('utoken')
        resp = session.post(ses_url, data=ses_params, headers=headers, proxies=PROXIES, verify=CERT_VERIFY)
    else:
        headers['utoken']=''
        resp = session.get(ses_url, params=ses_params, headers=headers, proxies=PROXIES, verify=CERT_VERIFY)
    resp = resp.json()
...
def userinfo(utoken, userid):
    ...
    if(resp['msg']==u'获取成功'):
        ...
        logging.info("姓名:" + resp['data']['username'])
        logging.info("学校:" + resp['data']['school'])
        logging.info("完成次数:" + resp['data']['effectiveTimes'])
        logging.info("目标次数:" + resp['data']['targetTotalTimes'])
        logging.info("userid:" + resp['data']['userid'])
        ...
        return True, resp['data']['username'], resp['data']['school'], resp['data']['effectiveTimes'], resp['data']['targetTotalTimes']
    else:
...
def startrun(initloc, userid):
    ...
    ses_data = json.dumps({"initLocation": initloc, "type": "1", "userid": userid}, separators=(',',':'))
    ses_sign = sign(ses_data)
...
def rundetail(runid, userid):
    ...
    ses_data = json.dumps({"runid": runid, "userid": userid}, separators=(',',':'))
    ses_sign = sign(ses_data)
...
def adreport(userid):
    ...
    ses_data = json.dumps({"userid":userid, "adId":"9071033990491217", "type":"VideoAd"}, separators=(',',':'))
    ses_sign = sign(ses_data)
    ...
    if(resp['code']==200):
        ...
        logging.warning("广告上报失败:" + misc_name[index] + ", 返回code:" + str(resp['code']))
        return False
...
def misc(index):
    ...
    # 3 bjcallback
    # 4 discoveryinit
    misc_name = ["adsList", "toastIndex5", "apiConfig", "bjCallback", "discovery/init(Android Only)"]
    misc_url = ['/api/art/adsList', '/api/Toast/get_index/5', '/index.php/api/configuration/apiConfig', '/api/reg/bjCallback', '/api/discovery/init']
    misc_data = [{}, {}, {}, {}, {}]
    misc_method = [['POST','GET'], ['POST','GET'], ['GET','GET'], ['GET','POST'], ['FUCK','GET']]
    # [IOS, ANDROID]
    ...
    if(resp['code']==200):
        ...
        logging.warning("请求失败:" + misc_name[index] + ", 返回code:" + str(resp['code']))
        return False